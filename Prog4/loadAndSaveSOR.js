...
function updateScreen(ev){
    ...
    var returnSOR = readFile();
    console.log(objectCounter)
    if(returnSOR){
        objectCounter -= 1 ;
        console.log(returnSOR.vertices, returnSOR.indexes);
        var newSor = new SORObject(returnSOR.vertices, returnSOR.indexes, [1, 1, 0,objectCounter/255] );
        listOfObjects.push(newSor);
...
function SOR(objName, vertices, indexes, normals){
...
function saveSOR(ev){
    var fileName = prompt("Please name file", "File Name");
    if (fileName != null) {

        saveFile(new SOR(fileName, vertices, indexes, vertexNormal ))
        console.log("Saving file", fileName)
    }
    else{
        alert("Sorry Could Not get name!")
    }
}
...
function saveFile(SOR) {
    ...
    // Convert Normals to text
    for (var i = 0; i < SOR.normals.length; i ++) {
        text = text + "n " + SOR.normals[i][0] + " " + SOR.normals[i][1] + " " + SOR.normals[i][2] + "\n";
    }
    ...
    if (f) {
        ...
        r.onload = function(e) {
            ...
            while (1) {
                ...
                if (line.charAt(0) == "n") {
                    line = line.substr(2);
                    sorObject.normals.push(parseFloat(line.substr(0, line.indexOf(" "))));
                    line = line.substr(line.indexOf(" ") + 1);
                    sorObject.normals.push(parseFloat(line.substr(0, line.indexOf(" "))));
                    line = line.substr(line.indexOf(" ") + 1);
                    sorObject.normals.push(parseFloat(line));
                }
